...
public class RestaurantOrder implements Order {
    ...
    public boolean remove(String name){
        for (int i = 0; i < dishes.length; i++){
            if(dishes[i].getTitle().equals(name)){
                if(i == size_dishes){
                    dishes[i] = null;
                }
                else{
        ...
        for (int i = 0; i < drinks.length; i++){
            if(drinks[i].getTitle().equals(name)){
                if(i == size_drinks){
                    drinks[i] = null;
                }
                else{
    ...
    public Dish[] getDishes() {
        ...
        if (size_dishes != 0){
            for (int i = 0; i < size_dishes; i++) {
                for (int j = 0; j < 6; j++){
                    if (dish[j] != null) {
                        if (dish[j].getTitle().equals(dishes[i].getTitle())) {
                            flag = false;
                        }
                    }
                }
    ...
    public Drink[] getDrinks() {
        ...
        if (size_drinks != 0){
            for (int i = 0; i < size_drinks; i++) {
                for (int j = 0; j < 6; j++){
                    if (drink[j] != null) {
                        if (drink[j].getTitle().equals(drinks[i].getTitle())) {
                            flag = false;
                        }
                    }
                }
